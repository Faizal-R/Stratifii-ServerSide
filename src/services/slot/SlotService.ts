...
export class SlotService implements ISlotService {
  ...
  async createRuleAndGenerateSlots(ruleData: ISlotGenerationRule): Promise<IInterviewSlot[]> {
   ...
   console.log("slots", slots);
    ...
    console.log(`[SlotService] Created ${slots.length} slots for rule: ${rule._id}`);
    return slots 
  }

  // Step 2: Generate slots from the rule
 async generateSlotsFromRule(rule: ISlotGenerationRule): Promise<IInterviewSlot[]> {
    ...
    while (current <= end) {
      ...
      if (rule.availableDays.includes(day)) {
        ...
        while (slotStart < dayEnd) {
          const slotEnd = new Date(slotStart.getTime() + rule.duration * 60 * 1000);
          ...
          slotStart = new Date(slotStart.getTime() + (rule.duration+rule.buffer) * 60 * 1000);
        }
      }
