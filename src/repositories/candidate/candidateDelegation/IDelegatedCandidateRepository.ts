...
export interface IDelegatedCandidateRepository
  extends IBaseRepository<IDelegatedCandidate> {
  getCandidatesByJob(
    jobId: string,
    query?: FilterQuery<IDelegatedCandidate>
  ): Promise<IDelegatedCandidate[]>;
  getDelegatedJobsByCandidateId(
    candidateId: string
  ): Promise<IDelegatedCandidate[]>;
  ...
  markLastRoundAsFollowUpScheduled(delegatedCandidateId: string): Promise<void>;
}
